...
<?php } 
	...
	if ( $posts->posts ) {
	    ...
	    foreach ( $posts->posts as $post ) :
	        setup_postdata( $post );
			$product = '';
			$topic_selector = get_field( 'topic_selector', $post->ID );
	        $product_class = '';
	        		...
	        		<div class="right-section">
					 ...
					 <div class="mainSearch">
						...
						<span class="topic-selector">	
							<?php echo $topic_selector; ?></span> <?php $this->date_markup( $field, $post ); ?>
						</p>
					</div>		
						
			        
					<h3 class="front-latest-title-search">
						  <?php $this->title_markup( $field, $post, $product ); ?>	
					</h3>	  					 
					

						<style>
					.front-latest-title-search .is-title a{
						font-size: 16px;
